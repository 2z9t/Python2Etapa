...
Existem 3 classes (base.py, filme_favorito.py, historico_busca.py)
base.py -> ModeloBase
...
A superclasse que herdam estas 2 subclasses é a base.py
id, data_criação e data_atualização
...
eu diria que seria tanto quanto um bom hábito, quanto para estruturar corretamente
o projeto e no quesito de organização do DB + inicialização da tabela
...
Sim, possui restrições especiais
...
Ele instancia o objeto com os seus parâmetros mais o self, após isso ele verifica
se o id já existe a partir de uma query de uma função já existente (buscar_por_tmdb)
e após isso se ele não encontrar, ele adiciona o projeto na tabela de filme_favorito
a partir de um add + commit e retornando logo em seguida

**8.** Onde está o método que lista as últimas 8 buscas? Qual é o nome da classe e do método?

H:\_python\flask\Aula12 - Alunos\models\historico_busca.py
O nome da classe pai é HistoricoBusca que tem herança sobre o filho que lista,
que é o @classmethod ultimas

**9.** O model grava dados da API TMDB inteira ou só alguns campos espelhados? Cite 4 campos salvos em `FilmeFavorito`.

O model logicamente grava todos os dados da API. tmdb_id, titulo, nota e ano
